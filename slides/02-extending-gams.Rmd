...
.pull-right[
```{r biom-space-plot, fig.height=8, echo=FALSE}
ggplot(shrimp2010) +
...
.pull-left[
```{r plot-effect-bad, fig.height=8}
plot(b_shrimp, asp=1)
```
]

.pull-right[
```{r plot-effect-good, fig.height=8}
...
b_shrimp_ll_ti <- gam(shrimp ~ offset(log(area_trawled)) +
                        ti(temp_bottom, depth) + ti(temp_bottom) + ti(depth),
                data=shrimp2010,
                family=quasipoisson,
                method="REML")
...
```{r biom-space-time-plot, fig.height=5}
ggplot(shrimp) +
  geom_point(aes(x=long, y=lat, size=shrimp)) +
  facet_wrap(~year)
  coord_map()
